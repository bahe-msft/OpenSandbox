...
// activityRoutes uses Gin route templates so instrumentation stays declarative
// and additions are reviewable alongside router.go.
var activityRoutes = map[activityRoute]activityMode{
...
func activityMiddleware(tracker *activity.Tracker) gin.HandlerFunc {
	if tracker == nil {
		panic("activity middleware tracker must not be nil")
	}
	return func(ctx *gin.Context) {
		mode := classifyActivity(ctx.Request.Method, ctx.FullPath(), ctx.Request.URL.Path, ctx.GetHeader("Upgrade"))
		switch mode {
		case activityBusy:
			end := tracker.Begin()
			defer end()
...
func classifyActivity(method, routePath, requestPath, upgrade string) activityMode {
	...
	return activityRoutes[activityRoute{method: method, path: routePath}]
}
